...
$contact_number = ucwords(mysqli_real_escape_string($conn, $_POST['contact_number']));
$date = (mysqli_real_escape_string($conn, $_POST['date']));
$location = ucwords(mysqli_real_escape_string($conn, $_POST['location']));
$location = stripslashes($location);
$cause = ucwords(mysqli_real_escape_string($conn, $_POST['cause']));
$cause = stripslashes($cause);
$police_station = ucwords(mysqli_real_escape_string($conn, $_POST['police_station']));
$police_station = stripslashes($police_station);
$case = ucwords(mysqli_real_escape_string($conn, $_POST['case']));
...
$sql = "INSERT INTO damage_claim (reference, company_name, email, contact_name, address, contact_number, date, prescription, location, cause, police_station, `case`, third_party_name, third_party_contact, third_party_address, third_party_email, details) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$html_template = file_get_contents('../16_insurance/damage_claim_form_2.html');
...
$html = str_replace('{contact_number}', $contact_number, $html);
$html = str_replace('{date}', $date, $html);
$html = str_replace('{location}', $location, $html);
$html = str_replace('{cause}', $cause, $html);
$html = str_replace('{police_station}', $police_station, $html);
$html = str_replace('{case}', $case, $html);
...
<div id="logo">
    ...
    <p>Kindly find attached hereto the completed Claim Form</p> 
    <p>Please sign the Claim Form and email a copy to: email@something.com.</p>
